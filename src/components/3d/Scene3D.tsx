...
function WalkthroughControls() {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (['w', 'a', 's', 'd', 'q', 'e'].includes(key)) keysPressed.add(key);
    };
    const onKeyUp = (e: KeyboardEvent) => keysPressed.delete(e.key.toLowerCase());
    const onBlur = () => keysPressed.clear();
  ...
  useEffect(() => {
    ...
    if (!canvas) return;
    const onMouseDown = (e: MouseEvent) => { if (e.button === 0) { isDragging.current = true; lastMouse.current = { x: e.clientX, y: e.clientY }; canvas.style.cursor = 'grabbing'; } };
    ...
    return () => { canvas.removeEventListener('mousedown', onMouseDown); window.removeEventListener('mouseup', onMouseUp); window.removeEventListener('mousemove', onMouseMove); };
  }, []);
...
export const Scene3D: React.FC = () => {
  ...
  return (
    <div
      className={`flex-1 bg-gray-200 relative ${walkthrough ? 'walkthrough-canvas' : ''}`}
      ...
      >
        <Suspense fallback={null}>
          ...
          <directionalLight position={[30, 40, 20]} intensity={1} castShadow
            ...
            shadow-camera-far={100} shadow-camera-left={-50} shadow-camera-right={50}
            shadow-camera-top={50} shadow-camera-bottom={-50}
          />
          <hemisphereLight args={['#b1e1ff', '#b97a20', 0.3]} />
          <GroundPlane />
          <Grid args={[200, 200]} cellSize={1} cellThickness={0.5} cellColor="#c0c0c0"
            sectionSize={5} sectionThickness={1} sectionColor="#a0a0a0"
            fadeDistance={100} position={[0, 0.001, 0]}
          />
          <ObjectMapper />
          {walkthrough ? <WalkthroughControls /> : (
            <OrbitControls makeDefault maxPolarAngle={Math.PI / 2 - 0.05} minDistance={2} maxDistance={150} />
          )}
        </Suspense>
      </Canvas>
      {contextMenu && (
